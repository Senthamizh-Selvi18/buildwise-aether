...
# room type -> small functional nudge (hue_bias(optional), saturation_delta, lightness_delta)
# Keeps recommendations architecturally sensible: e.g. pooja rooms lean
# toward purity/light, kitchens toward a touch more energy/cleanliness.
ROOM_TYPE_NUDGE: Dict[str, Tuple[Optional[float], float, float]] = {
    "kitchen": (None, 4, -1),
    "bathroom": (200, 2, 3), "common_bathroom": (200, 2, 3), "guest_bathroom": (200, 2, 3),
    "bedroom": (None, -6, 3), "master_bedroom": (None, -6, 3), "guest_bedroom": (None, -6, 3),
    "pooja": (48, -10, 10), "pooja_room": (48, -10, 10),
    "office": (None, -4, 1), "study": (None, -4, 1),
    "living_room": (None, 0, 0), "dining": (30, 2, 0),
    "entrance": (30, 4, -1), "corridor": (None, -3, 3), "staircase": (None, -2, 2),
    "home_gym": (10, 10, -4), "utility": (None, 0, 2), "storeroom": (None, -5, 2),
    "store_room": (None, -5, 2), "servant_room": (None, -4, 2), "balcony": (200, 2, 2),
    "parking": (None, -6, 0), "parking_slot": (None, -6, 0),
}
...
class PaintEngine:
    ...
    def generate(
        ...
        """
        Main entry point. `rooms`: [{"name": "master_bedroom", "area_sqft": 180}, ...].
    ...
    def _base_hsl(room_name: str, prefs: PaintPreferences) -> Tuple[float, float, float, float, List[str]]:
        ...
        if r_val:
            if r_val[0] is not None:
                hue = _blend_hue(hue, r_val[0], 0.10)
            sat += r_val[1]
            light += r_val[2]
        ...
        hue = (hue + (seed[0] / 255.0 - 0.5) * 14) % 360
        sat += (seed[1] / 255.0 - 0.5) * 10
...
def generate_recommendations_from_user_input(
    ...
    """Drop-in convenience wrapper mirroring the old function name/signature,
    now backed entirely by the generative engine."""
    clues = dict(answers or {})
    prefs = preferences_from_legacy_clues(clues, user_prompt=user_prompt, city=city, budget=budget)
    return PaintEngine.generate(rooms, prefs, allow_partial=allow_partial)
...
if __name__ == "__main__":
    ...
    for i, prefs in enumerate(scenarios, 1):
        ...
        result = PaintEngine.generate(room_set, prefs)
    ...
    empty_result = PaintEngine.generate(room_set, PaintPreferences())
